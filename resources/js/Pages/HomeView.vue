    ...
    <SectionMain>
      ...
      <div class="text-center mt-[80px] mb-6">
        <a class="text-white text-lg font-semibold bg-emerald-500 hover:bg-emerald-600 rounded-[30px] px-8 py-2"
          target="_blank" href="https://chat.whatsapp.com/FVMmkYNHEva0zP5xmWOTxC"><i class="fab fa-whatsapp pr-2 text-xl"></i>
      ...
      <CardBox has-table class="mb-5" v-if="has_current_savings">
        <div class="mx-3 my-5 text-sm">
          <h3 class="mb-3 text-2xl font-bold">Go-easy Savings Plan</h3>
          
          <p class="font-bold text-primary mb-4 mt-2">
            <span>You currently have a savings plan onging.</span>
          </p>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                <td></td>
                <td data-label="Pledged Amt.">₦{{ mainStore.addCommas(saving.amount) }}</td>
                <td data-label="Frequency">{{ saving.frequency }}</td>
                <td data-label="Duration">{{ saving.duration }}</td>
                <td data-label="Start Date">{{ saving.start_date }}</td>
                <td data-label="End Date">{{ saving.end_of_savings_date }}</td>
                <td data-label="Amt. Saved So Far">₦{{ mainStore.addCommas(saving.total_amount_debited_so_far) }}</td>
              </tr>
        ...
        <div class="float-right">
          <a class="text-emerald-600 text-sm underline font-bold" :href="route('view_saving_details',saving.id)">See more details</a>
        </div>
      </CardBox>
...
const has_current_savings = page.props.value?.has_current_savings;
const saving = page.props.value?.saving;
