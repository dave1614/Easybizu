<template inheritAttrs="false">
	<LayoutAuthenticated>
		...
		<Head title="Auto Withdrawal History" />
		<SectionMain>
			<SectionTitleLineWithButton :icon="mdiClipboardTextClock" title="Auto With. Hist" main>
				<BaseButton :href="route('goeasy_savings_main_page')" :icon="mdiBackburger" label="Go Back" color="warning"
					rounded-full small />
							...
							<tr v-for="(row, index) in history.data" :key="row.id">
								...
								<td class="before:hidden lg:w-1 whitespace-nowrap">
									<BaseButtons type="justify-start" no-wrap>
										<BaseButton title="View More Saving Information" color="info" :icon="mdiInformation" small :href="route('view_saving_details', row.easy_savings_id)" />
...
const submitFilterForm = () => {
	...
	let queryRoute = route('savings_auto_withd_history');
