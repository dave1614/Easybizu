<template inheritAttrs="false">
	<LayoutAuthenticated>
		...
		<Head title="Saving Debit History" />
		<SectionMain>
			<SectionTitleLineWithButton :icon="mdiClipboardTextClock" title="Saving Debit History" main>
				<BaseButton :href="route('view_saving_details',saving_id)" :icon="mdiBackburger"
          label="Go Back" color="warning" rounded-full small />
        ...
        <b>Responsive table.</b> Collapses on mobile
			...
			<CardBox isForm @submit.prevent="submitFilterForm" class="">
				<div class="sm:grid sm:grid-cols-12 sm:gap-6">
					...
					<FormField class="sm:col-span-4" label="Debit Date">
...
const submitFilterForm = () => {
	...
	let queryRoute = route('savings_debit_history', saving_id);
