<template inheritAttrs="false">
	<LayoutAuthenticated>
		...
		<Head :title="`${user.name}'s Saving Debit History`" />
		<SectionMain>
			<SectionTitleLineWithButton :icon="mdiClipboardTextClock" :title="`${user.name}'s Saving Debit History`" main>
		...
		<FloatingActionButton :title="'Go Back'"
			@click="Inertia.visit(route('view_users_saving_details', [user.id,saving_id]))">
...
const submitFilterForm = () => {
	...
	let queryRoute = route('users_savings_debit_history', [user.id, saving_id]);
