<template inheritAttrs="false">
	<LayoutAuthenticated>
		...
		<Head :title="`${user.name}'s Auto Withdrawal History`" />
		<SectionMain>
			<SectionTitleLineWithButton :icon="mdiClipboardTextClock" :title="`${user.name}'s Auto With. Hist.`" main>
					...
					<table>
						...
						<tbody>
							<tr v-for="(row, index) in history.data" :key="row.id">
								...
								<td data-label="Date / Time" v-html="row.date_time">
			
								</td>
								<td class="before:hidden lg:w-1 whitespace-nowrap">
									<BaseButtons type="justify-start" no-wrap>
										<BaseButton title="View More Saving Information" color="info" :icon="mdiInformation" small
											:href="route('view_users_saving_details',[user.id, row.easy_savings_id])" />
		...
		<FloatingActionButton :title="'Go Back'" @click="Inertia.visit(route('users_goeasy_savings_main_page', user.id))">
...
const submitFilterForm = () => {
	...
	let queryRoute = route('users_savings_auto_withd_history', user.id);
